...
// Only add a newline in case the current G-code does not end with a newline.
    ...
    std::string WipeTowerIntegration::append_tcr(GCode& gcodegen, const WipeTower::ToolChangeResult& tcr, int new_filament_id, double z) const
    {
        ...
        if (!is_approx(z, current_z)) {
            // LUGOWARE: Changed toolchange movement order - XY move first, then Z restore.
            // Instead of restoring Z here (before XY travel), we defer Z restore so the next
            // travel_to_xyz will move XY first then Z (when is_current_position_clear is false).
            gcode += gcodegen.writer().retract();
            // Update the writer's internal Z target without emitting a Z-only G-code move.
            // The next travel call will handle both XY and Z movement with XY first.
            Vec3d pos = gcodegen.writer().get_position();
            pos(2) = current_z;
            gcodegen.writer().set_position(pos);
            gcodegen.writer().set_current_position_clear(false);
    ...
    for (unsigned int extruder_id : layer_tools.extruders)
    {
        ...
        for (int print_wipe_extrusions = is_anything_overridden; print_wipe_extrusions>=0; --print_wipe_extrusions) {
            ...
            for (InstanceToPrint &instance_to_print : instances_to_print) {
                ...
                for (size_t island_idx = 0; island_idx < instance_to_print.object_by_extruder.islands.size(); ++island_idx) {
                    ObjectByExtruder::Island &island = instance_to_print.object_by_extruder.islands[island_idx];
                    ...
                    // Extrude each contour's walls, then match infill by bbox before next contour.
                    {
                        ...
                        for (size_t region_idx = 0; region_idx < by_region_specific.size(); ++region_idx) {
                            ...
                            for (ExtrusionEntity *ee : region.infills) {
                                if (ee->role() == erIroning) {
                                    m_config.apply(print.get_print_region(region_idx).config());
                                    gcode += this->extrude_entity(*ee, "ironing");
                                }
                            }
                        }
                    }
