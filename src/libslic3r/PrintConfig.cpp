...
void PrintConfigDef::init_fff_params()
{
    ...
    */
    ...
    def = this->add("filament_all_speed_override", coFloats);
    def->label = L("All speed override");
    def->tooltip = L("");
    def->sidetext = L("mm/s");
    def->min = 0;
    def->max = 1000;
    ...
    def->category = L("Strength");
    def->tooltip = L("Convert all solid infill (top, bottom, bridge, internal solid) to sparse infill. "
                     "Useful for flexible or foam structures where only sparse infill is desired.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def = this->add("fuzzy_skin_layer_step", coInt);
    def->label = L("Fuzzy skin layer step");
    def->category = L("Others");
    def->tooltip = L("Apply fuzzy skin every N layers. Value 1 means every layer (default behavior). "
                     "Value 5 means fuzzy skin is applied every 5th layer, with normal printing on other layers. "
                     "Value 0 disables fuzzy skin.");
    def->min = 0;
    def->max = 200;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInt(1));
    ...
    def = this->add("heating_time", coInt);
    def->label = L("Heating time");
    def->tooltip = L("Heating time variable sent to Klipper. Accessible as {heating_time} in G-code templates.");
    def->sidetext = L("s");
    def->min = 1;
    def->max = 30;
    def->mode = comAdvanced;
...
//    def->min = 0;
    ...
    def = this->add("xy_hole_compensation_layer_step", coInt);
    def->label = L("X-Y hole compensation layer step");
    def->category = L("Quality");
    def->tooltip = L("Apply X-Y hole compensation every N layers. Value 1 means every layer (default behavior). "
